									...
									<fieldset>
										...
										<div class="form-group">
											<div class="col-md-10">
												...
												<label class="radio radio-inline">
													<input type="radio" name="customer_type" value="new_customer"  onclick="customer('new_customer');">New Customer </label>
											</div>
										</div>
									</fieldset>
								...
								<!--  ######################### Where Address  ############################  -->
									<fieldset>
										...
										<div class="form-group">
											...
											<div class="col-md-10">
												<input type="text" name="street_address"   class="street_address form-control" placeholder="Address"  id="address" required>
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-md-10">
												<input type="text" name="zip_code"   class="zip form-control"  placeholder="Zip"  id="zip" required>
											</div>
										</div>
									...
									<fieldset>
										<article class="col-sm-7">
											...
											<div class="form-group">
													<div class="col-md-10">
													  <label>
														<input name="get_yards" onclick="get_yard()"  value=""  type="checkbox" id="get_yards"> Get Yards  
													  </label>
													</div>
											</div>
											
											<div class="form-group" id="yard_box" style="display:none">
											<label class="col-md-2 control-label">yard</label>
											<div class="col-md-10">
												<input type="text" name="yard"  value="90012"  class="yard_zip form-control" id="" required readonly >
											</div>
										</div>
											...
											<!--div class="form-group">
												...
												<div class="col-md-10">
													<select  required=""  name="service_size"   class="form-control" id="yadrs_id">
														...
														<option class= "select-val_<?php echo $val->id;?>"  value="<?php echo $val->id;?>"> <?php echo $val->yards;?>  </option>
															<?php }
														?>		
												</select>
												</div>
...
function get_yard() {
var getAddredd = $("#address").val();
var getZip = $("#zip").val();
	if(getAddredd == '' && getZip == '') {
		alert("Please fill the address and zip code first");
		$('#get_yards').attr('checked', false);
		$("#yard_box").hide();
	 } else {
				$.ajax({
							url: 'bookings/yards',
							type:'post', 
							data :'address='+getAddredd+'&zip='+getZip,
							success:function(resps) {
									var obj = $.parseJSON(resps);
									//$("#yard_box").show();
									$.each(obj, function(key,value) {
										...
										if(code == 0) {
											...
											var size = value.size;
										     var select_id= $('.yard_zip').attr('id', code);
										     var ss= $('.yard_zip').val(size);
											 $("#yard_box").show();
										}
									});	
							}
...
//var yard_id = $(".yard_zip").val();
var yard_id = $(".yard_zip").attr('id');
	var tip_value = $("#tip_value").val();
	$.ajax({
			url: 'bookings/searchExtraService',
			type:'post', 
			beforeSend: function(){
